...
where $\mathbf{B}_{adj} \in \mathbb{R}^{h \times N \times N}$ are learnable bias matrices for each attention head $h$. The attention scores are computed using scaled dot-product attention with an additional L1 regularization term:

\begin{equation}
\mathcal{L}_{attn} = \lambda_{attn}\|\mathbf{A}_{dyn}\|_1 + \mathcal{L}_{pred}
\end{equation}
...
\begin{equation}
\mathbf{F}_{fused} = \sum_{i=0}^{n_{scales}-1} \alpha_i \mathbf{F}_i, \quad \boldsymbol{\alpha} = \text{softmax}(\mathbf{w})
\end{equation}

where $\mathbf{w} \in \mathbb{R}^{n_{scales}} are learnable parameters optimized end-to-end.
...
\begin{equation}
\mathbf{G} = \sigma(\text{MLP}_g(\mathbf{F}_{fused}))
\end{equation}

\begin{equation}
\hat{\mathbf{Y}} = \mathbf{G} \odot \mathbf{X}_{last} + (1 - \mathbf{G}) \odot \text{MLP}_p(\mathbf{F}_{fused})
\end{equation}
